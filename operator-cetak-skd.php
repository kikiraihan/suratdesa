...
include "koneksi.php";
$id_skd  =  mysqli_escape_string($config,$_GET['id_skd']);
$query   =  mysqli_query($config, " SELECT * FROM tbl_skd JOIN tbl_pegawai JOIN tbl_penduduk ON tbl_skd.id_pg = tbl_pegawai.id_pg AND tbl_skd.NIK=tbl_penduduk.NIK WHERE tbl_skd.id_skd='$id_skd'");
...
$no_surat_skd     = $data['no_surat_skd'];
$ket              = $data['ket'];
}
?>
...
$tahun = date ('Y');
$nomor = "NO:474.4/BGM-TKBL/".$no_surat_skd."/".$romawi."/".$tahun;
...
echo "<table align='center' border='0' width='100%' height='100%'>
        <tr>
            <td align='center'>
                <u style='font-size:20px; font-weight:bold;' align='center'>SURAT KETERANGAN DOMISILI</u>
            </td>
        </tr>
        <tr>
...
echo  "<br><table border='0' style='margin-left:60px;' width='100%' height='100%'>
   ...
   echo "<table style='margin-left:60px;' width='100%' height='100%'>
                <tr>
                  <td colspan='3'><br>
                      <label style='font-size:16px;'>Demikian surat keterangan domisili ini dibuat dan diberikan kepada yang bersangkutan</label>
                  </td>
                </tr>
          </table>";
