...
$ket = $_POST['ket'];
	if(empty($ket)){
		echo "<script>window.alert('Keterangan surat belum di isi !'); window.location.href='skd.php'</script>";
	}else{
		$tambah = mysqli_query($config, "INSERT INTO tbl_skd VALUES ( '$_POST[id_skd]', '$_POST[nama]', '$_POST[nama_p]' , '$_POST[no_surat_skd]' , '$_POST[ket]' )");
			...
			if(!$tambah){
				echo "<script>window.alert('Data gagal di simpan !'); window.location.href='skd.php'</script>";
			}
		}
	}
?>